...
class DeepNovoTrainDataset(BaseDataset):
    def _get_feature(self, feature: DDAFeature) -> TrainData:
        ...
        peptide_id_list = [config.vocab[x] for x in feature.peptide] # > AA to AA's idx
        forward_id_input = [config.GO_ID] + peptide_id_list # add start
        forward_id_target = peptide_id_list + [config.EOS_ID] # add end
